...
module Ltk
  class LineEditPainter < WidgetPainter
    delegate cursor_position, cursor_rect, cursor_visible?, focused?, text_translate, to: widget

    def initialize(@widget : LineEdit, @painter : Painter)
    end
    ...
    def draw_cursor!
      if cursor_visible? && cursor_position >= 0
        ...
        if cursor_rect.width > 1
          puts "rect.width > 0 : #{cursor_rect.width}"
        else
